  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5 p-2 border border-gray-300 rounded-lg bg-white min-h-[42px]">
      {value.map((chip, i) => (
        <span
          key={i}
          className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-shift-pink text-white text-md font-medium"
        >
          {chip}
          <button
            onClick={() => removeChip(i)}
            className="hover:bg-shift-pink rounded-full w-4 h-4 flex items-center justify-center cursor-pointer"
          >
            &times;
          </button>
        </span>
